...
* [Jun Chen*, Thomas Wiegelmann, Li Feng*, Chaowei Jiang, & Rui Liu. (2026). FastQSL 2: A comprehensive toolkit for magnetic connectivity analysis. SCIENCE CHINA Physics, Mechanics & Astronomy, 69(8), 289611.](https://www.sciengine.com/SCPMA/doi/10.1007/s11433-025-2982-2)
...
* please specify the path of fastqsl.x, 
  * in fastqsl\.pro, please correct the line
    ...
    ```
  * in fastqsl\.py, please correct the line
    ```python
...
### Coordinates
  ...
  * **xa, ya, za**: axis coordinates of magnetic field in 1D arrays
    * should not be presented if the **Bx, By, Bz** are set on a Cartesian uniform grid
      ...
      ```
    * the size of **xa, ya, za** must be consistent with the size of the 3D magnetic field
  ...
  * **spherical**: whether the magnetic field is described on a spherical grid. 
    ...
    * The classical spherical coordinates are $\{r, \theta, \varphi\}$, **which are not the spherical coordinates for FastQSL!**  The maximum range of $\theta$ is $[0, \pi]$. If you take a magnetic field on a grid with the classical spherical coordinates, two relations should be applied:
      * $\vartheta=\pi/2-\theta$
      * $B_\vartheta=-B_\theta$
    * For example, in an IDL script, if the arrays of $B_r(r, \theta, \varphi), B_\theta(r, \theta, \varphi), B_\varphi(r, \theta, \varphi), r, \theta, \varphi$ are `br, bp, bt, radius, theta, phi`, they should be converted to $B_\varphi(\varphi, \vartheta, r), B_\vartheta(\varphi, \vartheta, r), B_r(\varphi, \vartheta, r), \varphi, \vartheta, r$ in arrays of `B_lon, B_lat, B_r, lon_rad, lat_rad, radius`. The following code can give **q** at the bottom:
      ```
  ...
  * **xperiod, yperiod, zperiod**: whether the $x, y, z$ -directions are periodic. For example, if **yperiod** is invoked, the period of y-direction is `ya[ny-1]-ya[0]` (if stretchFlag is invoked), the field line tracing will not stop at ymin, ymax. And the layer of `Bx[*,0,*]` will be copied to `Bx[*,ny-1,*]` in fastqsl.x (also for `By, Bz`).
    * defaults are 0
    * can only work with Cartesian coordinates. For spherical coordinates, $\varphi$-direction will be automatically periodic if `ya[ny-1]-ya[0]` is close enough to $2 \pi$
    * Normally, FastQSL requires at least 3 layers for every direction to compute $\nabla \times \vec{B}$ and $\nabla \dfrac{\vec{B}}{B}$. If you need to analyze a field with the symmetry like 2D, e.g. $\dfrac{\partial \{B_x, B_y, B_z\}}{\partial y}=\{0,0,0\}$, just stack $\vec{B}(x,z)$ for 2 layers by
      ```
      ...
      * For spherical coordinates, ny, nz cannot be 2, because $\vartheta, r$-directions cannot be periodic in any case
### Output domain
  * **xreg, yreg, zreg**: coordinates of the output region, in arrays of two elements
    ...
    * If csFlag is set, see below
      * invoke cFlag
    * in return, `qsl.xreg[1], qsl.yreg[1], qsl.zreg[1]` may be slightly changed, which is the same as the final grid of `qsl.seed`, due to the trim at the final grid
  ...
  * **csFlag**: 
    * the output domain is a cross section defined by three points:
      ...
      * point0 -> point2 is the second axis, and 
        ...
        ```  
    * `xreg[0], yreg[0], zreg[0]` do not have to be smaller than `xreg[1], yreg[1], zreg[1]` in this case
    * default is 0
...
### Output files
  ...
  * **tmp_dir**:     the temporary directory for the data transmission between fastqsl.x and fastqsl\.pro
    ...
    * a virtual disk created with computer memory provides an incredibly fast IO performance, setting tmp_dir on such disk is suggested
...
Possible elements in **qsl** are:
  ...
  * **rboundary**:  nature of the terminal points
    * rboundary is given by 10*rbs+rbe in fastqsl.x, therefore:
      ...
      ```
      their values mark where **rFs, rFe** are terminated:
...
* Jan 17, 2023 Jun Chen, integrate doppler color in qfactor\.pro, doppler_color\.pro is not necessary anymore;
            to avoid an error in a remote server: % TVLCT: Unable to open X Windows display
