...
### Coordinates
  ...
  * **xa, ya, za**: axis coordinates of magnetic field in 1D arrays
    * They do not need to be presented if the **Bx, By, Bz** are set on a Cartesian uniform grid
      ```
...
A magnetic field line is integrated using $\dfrac{\textrm{d} \vec{r}(s)}{\textrm{d} s}=\dfrac{\vec{B}}{B}$
  ...
  * **inclineFlag**: to apply Equation (20) or (21) in [Zhang (2022)]((https://iopscience.iop.org/article/10.3847/1538-4357/ac8d61)):
    ...
    $\texttt{step}|_{S0} = \textrm{max}([\texttt{step}_{\perp} \times |B_{n,0} / B|, \texttt{step}_\mathrm{min}])$

    $ \texttt{tol}|_{S0} = \texttt{tol}_{\perp} \times | B_{n,0} / B |^{1.5}$

    $\textrm{tol}|_{S0}= \textrm{tol}_{\perp} \times | B_{n,0} / B |^{1.5}$
