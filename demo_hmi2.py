...
"""
import pfsspy, sunpy, os, subprocess
...
def get_HMI_magnetogram(cr_target,data_dir):
	...
	if target_filename not in os.listdir(data_dir):
		subprocess.run(r'wget http://jsoc.stanford.edu/data/hmi/synoptic/'+target_filename, shell=True)
	if target_filename in os.listdir(data_dir):
		...
		return ""
# ------------------------------------------------------------
target_CR=2285	#Carrington rotation
...
# fname='r_cut2', preview=True, keep_tmp=True)

# fastqsl(b_lon[r_cut:,:,:], b_lat[r_cut:,:,:], b_r[r_cut:,:,:], \
# xa=lon_rad, ya=lat_rad, za=radius[r_cut:], spherical=True, \
# fname='r_cut2r2.5', zreg=[2.5,2.5], preview=True)

qsl=fastqsl(b_lon[r_cut:,:,:], b_lat[r_cut:,:,:], b_r[r_cut:,:,:], \
xa=lon_rad, ya=lat_rad, za=radius[r_cut:], spherical=True, \
fname='spherical_seed_path', preview=False, \
twist_out=True, length_out=True, \
seed=[[np.pi*0.85, 0.1, 1.7], [np.pi*1.1, -0.2, 1.2]], \
